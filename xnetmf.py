...
def get_khop_neighbors(graph, emb_method):
	if emb_method.max_layer is None:
		emb_method.max_layer = graph.N #Don't need this line, just sanity prevent infinite loop
	...
	for node in range(graph.N):
		neighbors = np.nonzero(graph.adj[node])[-1].tolist() ###
	...
	while True:
		if emb_method.max_layer is not None and current_layer > emb_method.max_layer: break
...
def get_features(graph, emb_method, verbose = True):
	...
	if verbose:
		print("max degree: ", graph.max_features["degree"])
	...
	if emb_method.num_buckets is not None:
		num_features = int(math.log(graph.max_features["degree"], emb_method.num_buckets) + 1)
	else:
		num_features = int(graph.max_features["degree"]) + 1
	...
	for n in range(num_nodes):
		for layer in graph.khop_neighbors[n].keys(): #construct feature matrix one layer at a time
			if len(graph.khop_neighbors[n][layer]) > 0:
				...
				deg_seq = get_degree_sequence(graph, emb_method, graph.khop_neighbors[n][layer], n)
...
def compute_similarity(graph, emb_method, vec1, vec2, node_attributes = None, node_indices = None):
	dist = emb_method.gammastruc * np.linalg.norm(vec1 - vec2) #compare distances between structural identities
	if graph.node_attributes is not None and emb_method.use_attr_dist:
...
def get_sample_nodes(graph, emb_method, verbose = True):
	#Sample uniformly at random
	sample = np.random.permutation(np.arange(graph.N))[:emb_method.dimensionality]
	return sample
...
def get_feature_dimensionality(graph, emb_method, verbose = True):
	d = int(emb_method.k*math.log(graph.N, 2)) #k*log(n) -- user can set k, default 10
	if verbose:
		print("feature dimensionality is ", min(d, graph.N))
	emb_method.dimensionality = min(d,graph.N)  #don't return larger dimensionality than # of nodes
	return emb_method.dimensionality

#xNetMF pipeline
def get_representations(graph, emb_method, verbose = True):
	#Node identity extraction
	feature_matrix = get_features(graph, emb_method, verbose)
	...
	if emb_method.landmark_features is None or not emb_method.use_landmarks:
		print("Getting landmark features...")
		if emb_method.dimensionality is None:
			emb_method.dimensionality = get_feature_dimensionality(graph, emb_method, verbose = verbose)
		elif emb_method.dimensionality > graph.N: 
			print("Warning: dimensionality greater than number of nodes. Reducing to n")
			emb_method.dimensionality = graph.N
		landmarks = get_sample_nodes(graph, emb_method, verbose = verbose)
	...
	C = np.zeros((graph.N,emb_method.dimensionality))
	for node_index in range(graph.N): #for each of N nodes
		for landmark_index in range(emb_method.dimensionality): #for each of p landmarks
			#select the p-th landmark
			
			if emb_method.landmark_features is not None: #landmarks are hard coded as actual features of landmark nodes
			...
			C[node_index,landmark_index] = compute_similarity(graph, 
															...
															(node_index, landmark_index))


	before_computerep = time.time()

	#Compute Nystrom-based node embeddings
	if emb_method.l2l_decomp is None or not emb_method.use_landmarks:
		...
		if verbose:
			print("Using saved decomposition of l2l similarity matrix")
	reprsn = np.dot(C, emb_method.l2l_decomp)
